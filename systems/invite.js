...
async function tryDetect(member, base, baseText) {
  ...
  if (deleted.length > 1) {
    console.log(
      `[INVITE] ${baseText} ${deleted.length} invites deletados e nenhum uses alterado. Sem detecção confiável.`
    );
  } else {
    console.log(
      `[INVITE] ${baseText} NENHUMA mudança de uses encontrada.`
    );
  }
  return null;
}
...
function recheckPending() {
  ...
  for (const [memberId, entry] of pendingJoins) {
    ...
    tryDetect(entry.member, inviteCache, "[INVITE] (recheck)").then((used) => {
      if (used) {
        pendingJoins.delete(memberId);
